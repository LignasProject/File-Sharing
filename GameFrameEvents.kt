...
class GameFrameEvents : PluginEvent() {
    ...
    private fun processIfCloseModal(player: Player) {
        if (player.ui.closeModal) {
            ...
            // esc_to_close_desktop: 0 = enabled (default), 1 = disabled.
            // CloseMainComponent is sent by the client on Esc press.
            if (player.getVarbit("varbits.esc_to_close_desktop") != 0) {
                return
            }
            player.ifClose()
            // Also close any overlay on the floater (e.g. settings panel) so that
            // Esc properly closes overlay interfaces when esc-to-close is enabled.
            val floater = Component("components.toplevel_osrs_stretch:floater".asRSCM())
            player.closeSubs(floater)
        }
    }
